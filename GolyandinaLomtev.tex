...
Класс разделимых рядов описан в \cite{Usevich2010}.
...
\begin{lemma}
\label{svd_YY_}
	Пусть $\matr{Y} \in \cM_{L,K}$, $\func{rank}\matr{Y}=r$, $\tau \leq \lfloor\frac{K}{2}\rfloor$. Рассмотрим некоторое сингулярное разложение матрицы $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]=\matr{U}\matr{\Lambda}^{1/2}\T{\matr{T}}$,
...
\marginpar{С $\tau \leq K - r$ было бы верно только если $\matr{Y}$ траекторная ряда ранга $r$. Так верно для любой. Или поставить условие, что ранг $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]$ равен r}
...
\begin{lemma}
	Пусть матрица $\matr{Q} \in \cM_{K,r}$, и $\matr{C}_{\tau}\bracket{\matr{Q}}=\matr{W}\matr{D}\T{\matr{W}}$ --- некоторое спектральное разложение $\matr{C}_{\tau}\bracket{\matr{Q}}$. Тогда $\matr{D}=\matr{C}_{\tau}\bracket{\matr{S}}$, где $\matr{S}=\matr{Q}\matr{W}$, и, следовательно, $\matr{C}_{\tau}\bracket{\matr{S}}$ --- диагональная матрица.
\label{C_S_diag}
\end{lemma}
\begin{proof}
...
Пусть $\matr{Y}, \matr{Q}, \matr{\Lambda}, \matr{U}$ определены в лемме \ref{svd_YY_}, а $\matr{W}$ и $\matr{S}=[S_1:\ldots:S_r]$ определены в лемме \ref{C_S_diag}. Тогда $\matr{Y}=\sum\limits_{i=1}^{r}\hat{U}_i \T{S}_i$, где $\hat{U}_i$ --- столбцы матрицы $\hat{\matr{U}}=\matr{U}\matr{\Lambda}^{1/2}\matr{W}$.
%, $\matr{W}$ из спектрального разложения $\matr{C}_{\tau}\bracket{\matr{Q}}=\matr{W}\matr{D}\T{\matr{W}}$, $\matr{Q}=\T{\matr{Y}}\matr{U}\matr{\Lambda}^{-1/2}$, $\matr{U}$ --- левые сингулярные векторы $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]$
\end{theorem}
\begin{proof}
...
\marginpar{Если $L \leq K$, то условие на $0 < \tau \leq \lfloor\frac{L-r}{2}\rfloor$}

\medskip
\textbf{Алгоритм SSA-AMUSE}

\textbf{Вход}: Матрица $\matr{Y}$ размера $L \times K$ ранга $r$, сдвиг $0 < \tau \leq \min \bracket{\lfloor\frac{L-r}{2}\rfloor, K-\tau, \lfloor\frac{K}{2}\rfloor}$.
\begin{enumerate}
    	\item Построим сингулярное разложение матрицы $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]$, которое по лемме \ref{svd_YY_} имеет вид
    	    ...
    	    \item Получаем $\matr{W}=\left[W_1:\ldots:W_r \right]$, где $W_i$, $i=1,\ldots,r$, --- собственные векторы матрицы $\matr{C}_{\tau}\bracket{\matr{\Q}}$.
        ...
        $
\wtilde\tY_N = \wtilde\tY^{(1)}_N+\ldots+\wtilde\tY^{(r)}_N,$ где $\wtilde\tY^{(k)}_N = \calT^{-1} \calH \matr{Y}_k.
$
\end{enumerate}
    \textbf{Результат}: разложение ряда $\wtilde\tY_N$ на сумму компонент $\wtilde\tY_N = \wtilde\tY^{(1)}_N+\ldots+\wtilde\tY^{(r)}_N$.
...
\subsection{Вспомогательные утверждения}
\label{SSA_ICA_prop}
Будем рассматривать ряды $\tX_N$, $\tX_N^{(1)}$ и $\tX_N^{(2)}$ конечного ранга, управляемые ЛРФ порядка $d$, $d_1$ и $d_2$ соответственно, и пространства строк $\gX^{(K,1)},\gX^{(K,2)}$ и $\gX^{(K)}$ их траекторных матриц.
...
%Как и раньше,  пусть $\gX^{(K,1)},\gX^{(K,2)}$ и $\gX^{(K)}$ --- пространства строк траекторных матриц рядов $\tX_N^{(1)}$, $\tX_N^{(2)}$ и $\tX_N$ %размерности, соответственно, $d_1$, $d_2$ и $d$. 
 Обозначим $\righttau{{\gX}^{(K,1)}},\:\righttau{{\gX}^{(K,2)}}$ и $\righttau{{\gX}^{(K)}}$ --- пространства строк траекторных матриц без последних $\tau$ столбцов, и также $\lefttau{{\gX}^{(K,1)}}$, $\lefttau{{\gX}^{(K,2)}}$ и $\lefttau{{\gX}^{(K)}}$ --- пространства строк траекторных матриц без первых $\tau$ столбцов.

\begin{lemma}
	\label{UpBottau_spaces}
	Если ряд $\tX_N$ управляется ЛРФ порядка $d$, $d \leq \min\bracket{K - \tau,L - 2\tau}$, то
	$
...
Рассмотрим базис $V_1,\ldots,V_d$ в $\gX^{(K)}$, где $V_i$ --- исходные столбцы матрицы $\matr{X}$. \marginpar{NG: чьи столбцы?}
Соответствующие ему векторы $\bottau{{V_1}},\ldots,\bottau{{V_d}} \in \righttau{{\gX}^{(K)}}$ линейно независимы, так как иначе их можно было бы достроить по ЛРФ до
зависимых векторов, совпадающих по однозначности с $V_1,\ldots,V_d$. Лемма доказана.
\end{proof}

\marginpar{поправил. Если в условиях леммы \ref{UpBottau_spaces} в лемме \ref{svd_YY_} строки матрицы $\matr{Y}$ лежат в $\gX^{(K)}$, то лемма \ref{svd_YY_} выполняется.}
...
\label{one_tau_deter_cons}
	Пусть $\wtilde{\matr{S}}$ --- некоторая матрица ранга $d$ такая, что для фиксированного $\tau > 0$ выполняется $\matr{C}_{\tau}\bracket{\wtilde{\matr{S}}}=\func{diag}\bracket{\mu_1,\ldots,\mu_d}$, где $\mu_1 \geq \mu_2 \geq \ldots \geq \mu_d$,
   и $\wtilde{\matr{W}}$ --- некоторая ортогональная матрица . Обозначим $\wtilde{\matr{Q}}=\wtilde{\matr{S}}\T{\wtilde{\matr{W}}}$. Если для некоторых подмножеств $I_1 \subset \left\{ 1,\ldots, d\right\}$ и $I_2 \subset \left\{ 1,\ldots, d\right\}$ множества $\lbrace \mu_i, i \in I_1 \rbrace$ и $\lbrace \mu_j, j \in I_2 \rbrace$ не пересекаются, то для любого спектрального разложения матрицы $\matr{C}_{\tau}\bracket{\wtilde{\matr{Q}}}=\matr{H}\matr{D}\T{\matr{H}}$ с собственными числами, упорядоченными по убыванию, выполнено
	\begin{enumerate}
		\item $\matr{D}=\matr{C}_{\tau}\bracket{\wtilde{\matr{S}}}$,
...
\end{lemma}
\begin{proof}
	По лемме \ref{C_S_diag} матрицы $\matr{C}_{\tau}\bracket{\wtilde{\matr{Q}}}$ и $\matr{C}_{\tau}\bracket{\wtilde{\matr{S}}}$ подобны, поэтому у них 
\marginpar{NG: почему-то скобки слишком большие стали}
совпадают собственные числа, что доказывает первый пункт леммы. Также из подобия матриц следует, что столбцы матрицы $\wtilde{\matr{W}}$ являются ортонормированными собственными векторами матрицы $\matr{C}_{\tau}\bracket{\wtilde{\matr{Q}}}$.	
...
\lefttau{{\matr{Y}}^{(1)}}\T{\bracket{\righttau{{\matr{Y}}^{(2)}}}}\right]\matr{U}\!\matr{\Lambda}^{-1/2}\matr{W}^{(2)}/2$ и равно нулю по условию диагональности.
	...
	Достаточность. По условию \eqref{eq:wsep1} и лемме \ref{svd_YY_} существуют базисы $\matr{S}^{(1)} \in \gX^{(K,1)}, \matr{S}^{(2)} \in \gX^{(K,2)}$ такие, что столбцы матрицы $\Xi^{(1)}$ ортогональны друг другу и столбцам $\Xi^{(2)}$. \marginpar{NG: Не поняла, базисы $\matr{S}^{(1)}$, матрицы $\Xi^{(1)}$}
Отсюда можно вывести, что столбцы матрицы $\matr{S}=\left[ \matr{S}^{(1)}: \matr{S}^{(2)}\right]$ образуют базис $\gX^{(K)}$.	
	Из условия \eqref{eq:wsep2} следует, что матрица $\matr{C}_{\tau}\bracket{\matr{S}}$
	 диагональна. Далее рассмотрим выражения $\left[\!\lefttau{{\matr{Y}}^{(i)}}:\righttau{{\matr{Y}}^{(i)}}\!\right]=\left[\!\lefttau{{\matr{Y}}}:\righttau{{\matr{Y}}}\!\right]\Xi^{(i)}\T{\bracket{\Xi^{(i)}}}$. Заменим $\left[\!\lefttau{{\matr{Y}}}:\righttau{{\matr{Y}}}\!\right]$ на сингулярное разложение $\matr{U}\matr{\Lambda}^{1/2}\left[\T{\bracket{\toptau{\matr{Q}}}}:\T{{\bottau{\matr{Q}}}}\right]$, при этом $\matr{Q}=\matr{S}\T{\matr{W}}$ для некоторой ортогональной матрицы $\matr{W}=\left[ \matr{W}^{(1)}:\matr{W}^{(2)} \right]$, $\matr{S}^{(i)}=\matr{Q}\matr{W}^{(i)}$. После подстановки можно вывести $\left[\!\lefttau{{\matr{Y}}^{(i)}}:\righttau{{\matr{Y}}^{(i)}}\!\right]=\matr{U}\matr{\Lambda}^{1/2}\matr{W}^{(i)}\T{\bracket{\Xi^{(i)}}}$. Откуда по теореме \ref{SSA_AMUSE_decomp} $\matr{Y}^{(i)}=\matr{U}\matr{\Lambda}^{1/2}\matr{W}^{(i)}\T{\bracket{\matr{S}^{(i)}}}$.
...
\marginpar{Новый вариант оформления}
...
Тогда сильная SSA-AMUSE разделимость равносильна тому, что множества
 собственных чисел матриц $\matr{C}_{\tau}\bracket{\matr{Q}^{(1)}}$ и $\matr{C}_{\tau}\bracket{\matr{Q}^{(2)}}$
не пересекаются.
%	\end{itemize}
\end{theorem}
\begin{proof}
...
Пусть $\tX^{(1)}$ и $\tX^{(2)}$ --- бесконечные ряды, управляемые ЛРФ порядка $d$. Если отрезки этих бесконечных рядов достаточно большой длины $N$ такой, что сдвиг $0 < \tau \leq \min \bracket{K-d, \lfloor\frac{L-d}{2}\rfloor}$,
слабо Basic SSA разделимы при длине окна $L$, то удлинённые на $\tau$ (то есть длины $N+\tau$) отрезки ряда будут слабо SSA-AMUSE разделимы при том же $L$ и сдвиге $\tau$. Также будут слабо SSA-AMUSE разделимы неудлинённые отрезки длины $N$ при длине окна $L-\tau$ и сдвиге $\tau$.
\end{proposition}
...
\begin{enumerate}
	...
	\label{sinsin_str_ex}
	Рассмотрим $\tX^{(1)}=(\cos \pi n, n=1,\ldots,N)$ и $\tX^{(2)}=(\func{const}, n=1,\ldots,N)$. Если $K$ нечётно, то пространства $\lefttau{\gX^{(K,1)}}$ и $\lefttau{\gX^{(K,2)}}$ не ортогональны, однако условия \eqref{eq:wsep1} и \eqref{eq:wsep2} выполнены. Таким образом, данные ряды слабо SSA-AMUSE разделимы.
	
	\item Получим условия сильной разделимости двух гармоник.
	Рассмотрим ряды $\tX^{(1)}$ и $\tX^{(2)}$ длины $N$ с общими членами в виде $x^{(i)}_n=A_i\sin \bracket{2\pi n \omega_i + \gamma_i}$, $n=1,\ldots,N$, $0<\omega_i \leq 0.5$, $\omega_1\neq\omega_2$. Докажем, что если $(K-1)\omega_1$ и $(K-1)\omega_2$ целые, то ряды сильно SSA-AMUSE разделимы. Слабая разделимость следует из $\lefttau{\gX^{(K,1)}} \perp \lefttau{\gX^{(K,2)}}$. Сильную разделимость можно доказать  следующим образом. Пусть для определенности $\omega_i<0.5$. В этом случае найденный на шаге 3 алгоритма SSA-AMUSE базис можно разбить на базисы пространств $\gX^{(K,1)}$ и $\gX^{(K,2)}$ вида:
	\begin{gather*}
...
Заметим, что в случае применения алгоритма SSA-AMUSE полученные на шаге 3 базисные вектора будут упорядочены по значениям $\cos\bracket{2\pi \omega_i}$, в то время как в Basic SSA они упорядочены по амплитудам $A_i$. Упорядоченность по $A_i$ приводит к смешиванию в случае совпадающих амплитуд.
