...
для разделимости методом Basic SSA, так как утверждения \ref{prop:sep_w} и \ref{prop:sep_s} дают возможность двух эквивалетных определений слабой и сильной разделимости.
...
%	
	Используя определение разделимости \ref{def:sep_w},
\marginpar{Здесь лучше "используя определение разделимости \ref{def:sep_w}", условия разделимости мы вроде как выводим}	
	 можно показать, что матрица $\matr{P}^{(i)}\!=\!\matr{U}\!\matr{\Lambda}^{1/2}\matr{W}^{(i)}\!\T{\bracket{\!\matr{W}^{(i)}\!}}\!\!\matr{\Lambda}^{-\frac{1}{2}}\T{\matr{U}}$ задает ортогональный проектор на пространство столбцов матрицы $\matr{Y}^{(i)}$.
...
Пусть $\tX^{(1)}$ и $\tX^{(2)}$ --- бесконечные ряды, управляемые ЛРФ порядка $d$. Если отрезки этих бесконечных рядов достаточно большой длины $N$ такой, что сдвиг $0 < \tau \leq \min \bracket{\lfloor\frac{L-r}{2}\rfloor, K-\tau, \lfloor\frac{K}{2}\rfloor}$, \marginpar{check}
