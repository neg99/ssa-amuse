...
\marginpar{В \cite[глава 5]{Golyandina.etal2001} $L \geq d$ и $d \leq \min \bracket{L,K}$}
...
\marginpar{Убрал столбцы $Y_k$. Они далее не встречаются} 
...
Положим
	\begin{gather}
	\label{C_tau}
\matr{C}_{\tau}\brackettt{\matr{B}} = \left[\T{\bracket{\toptau{\matr{B}}}}\bottau{\matr{B}}+\T{\bracket{\bottau{\matr{B}}}}\toptau{\matr{B}}\right]\! \big/ 2\: \in \cM_{B,B}.
	\end{gather}

\marginpar{Наверное лучше привести все $\frac{x}{y}$ к виду $x/y$ для единого стиля}
...
Пусть $\matr{Y}, \matr{Q}, \matr{\Lambda}, \matr{U}, r$ определены в лемме \ref{svd_YY_} и $\tau\! \leq\! K/2$, а $\matr{W}$ и $\matr{S}=[S_1:\ldots:S_r]$ определены в лемме \ref{C_S_diag}. Тогда $\matr{Y}=\sum\limits_{i=1}^{r}\hat{U}_i \T{S}_i$, где $\hat{U}_i$ --- столбцы матрицы $\hat{\matr{U}}=\matr{U}\matr{\Lambda}^{1/2}\matr{W}$.
...
Пусть $\tX^{(1)}_{N+\tau}$ и $\tX^{(2)}_{N+\tau}$ --- ряды, управляемые ЛРФ порядка $d < N/2$, при этом ряды $\tX^{(i)}_{N}$ из первых $N$ элементов $\tX^{(i)}_{N+\tau}$ слабо Basic SSA разделимы при длине окна $L$. И пусть $\tau,L,K=N-L+1$ и $d$ такие, что $\tau \leq \min \bracket{K/2, K\!-\!d, \bracket{L-d}\!/{2}}$. Тогда 
\begin{enumerate}
...
А именно, гармоники $\tX^{(1)}$ и $\tX^{(2)}$  с общими членами в виде $x^{(i)}_n\!=\!A_i\sin\! \bracket{2\pi n \omega_i + \gamma_i}$, $n=1,\ldots$, являются асимптотически сильно разделимыми даже при совпадающих
