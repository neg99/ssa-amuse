...
где $\matr{\Lambda}^{1/2}\in \cM_{r,r}$ --- диагональная матрица с сингулярными числами на диагонали,
...
\begin{proof}
	...
	Так как в матрице $\righttau{\matr{X}}$ строки с $\tau+1$ по $L$ совпадают со строками с первой по $L-\tau$ матрицы $\lefttau{\matr{X}}$, то для доказательства равенства подпространств остаётся показать, что строки с первой по $\tau$ матрицы $\righttau{\matr{X}}$ принадлежат пространству $\lefttau{{\gX}^{(K)}}$ и строки с $L-\tau+1$ по $L$ матрицы $\lefttau{\matr{X}}$ принадлежат $\righttau{{\gX}^{(K)}}$. Действительно, это так, так как ряд управляется ЛРФ порядка $d \leq L-2\tau$, и указанные строки являются линейными комбинациями совпадающих строк.
...
\label{one_tau_deter_cons}
   ...
   и $\wtilde{\matr{W}}$ --- некоторая ортогональная матрица. Обозначим $\wtilde{\matr{Q}}=\wtilde{\matr{S}}\T{\wtilde{\matr{W}}}$. Если для некоторых подмножеств $I_1 \subset \left\{ 1,\ldots, d\right\}$ и $I_2 = \left\{ 1,\ldots, d\right\} \backslash I_1$ множества $\lbrace \mu_i, i \in I_1 \rbrace$ и $\lbrace \mu_j, j \in I_2 \rbrace$ не пересекаются, то для любого спектрального разложения матрицы $\matr{C}_{\tau}\brackettt{\wtilde{\matr{Q}}}=\matr{H}\matr{D}\T{\matr{H}}$ с собственными числами, упорядоченными по убыванию, выполнено
	\begin{enumerate}
...
\begin{proof}
	...
	Необходимость. В обозначениях теоремы \ref{SSA_AMUSE_decomp} существует $\matr{W}$ такая, что для $\matr{S}=\matr{Q}\matr{W}=\left[ S_1, \ldots, S_d \right]$ матрица $\matr{C}_{\tau}\brackettt{\matr{S}}$ диагональна, и базис $\left\lbrace S_1,\ldots, S_d \right\rbrace$ пространства $\gX^{(K)}$ можно разбить на подмножества $\left\lbrace S_1^{(1)},\ldots, S_{d_1}^{(1)} \right\rbrace$ и $\left\lbrace S_1^{(2)},\ldots, S_{d_2}^{(2)} \right\rbrace$ так, что первое составляет базис $\gX^{(K,1)}$, а второе --- базис $\gX^{(K,2)}$. Переупорядочим столбцы $\matr{S}$ и $\matr{W}$ так, что $\matr{S}^{(i)}=\left[ S_1^{(i)}:\ldots: S_{d_i}^{(i)} \right]=\matr{Q}\matr{W}^{(i)},\: i=1,2$, $\matr{S}=\left[ \matr{S}^{(1)}: \matr{S}^{(2)} \right]$, $\matr{W}=\left[ \matr{W}^{(1)}: \matr{W}^{(2)} \right]$. Обозначим $\boldsymbol\Xi^{(i)} = \T{\left[\!\T{\bracket{\toptau{{\matr{S}}^{(i)}}}}\!:\!\T{\bracket{\bottau{{\matr{S}}^{(i)}}}}\!\right]}$. Столбцы матрицы $\boldsymbol\Xi^{(1)}$ ортогональны столбцам $\boldsymbol\Xi^{(2)}$, при этом столбцы матрицы $\boldsymbol\Xi^{(i)}$ образуют ортогональный базис строк матрицы $\left[\!\lefttau{{\matr{Y}}^{(i)}}:\righttau{{\matr{Y}}^{(i)}}\!\right]$. Отсюда следует равенство \eqref{eq:wsep1}.
%	
...
\lefttau{{\matr{Y}}^{(1)}}\T{\bracket{\righttau{{\matr{Y}}^{(2)}}}}\right]\matr{U}\!\matr{\Lambda}^{-1/2}\matr{W}^{(2)}/2$ и равно нулю по условию диагональности.
	...
	Достаточность. По условию \eqref{eq:wsep1} и теореме \ref{SSA_AMUSE_decomp} существуют матрицы  $\matr{S}^{(1)}$ и $\matr{S}^{(2)}$, чьи столбцы образуют базисы $\gX^{(K,1)}$ и $\gX^{(K,2)}$, причём столбцы матрицы $\boldsymbol\Xi^{(1)}$ ортогональны друг другу и столбцам $\boldsymbol\Xi^{(2)}$ (матрицы $\boldsymbol\Xi^{(i)}$ определяются так же,
