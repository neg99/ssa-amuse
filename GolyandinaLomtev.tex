...
для разделимости методом Basic SSA, так как утверждения \ref{prop:sep_w} и \ref{prop:sep_s} дают возможность для двух эквивалетных определений слабой и сильной разделимости.
...
Положим
	\begin{gather}
	\label{C_tau}
\matr{C}_{\tau}\brackettt{\matr{B}} = \frac{1}{2}\left[\T{\bracket{\toptau{\matr{B}}}}\bottau{\matr{B}}+\T{\bracket{\bottau{\matr{B}}}}\toptau{\matr{B}}\right]\in \cM_{B,B}.
	\end{gather}
...
\textbf{Вход}: Матрица $\matr{Y}$ размера $L \times K$ ранга $r$, сдвиг $0 < \tau \leq \min \bracket{\lfloor\frac{L-r}{2}\rfloor, K-r, \lfloor\frac{K}{2}\rfloor}$.
\begin{enumerate}
...
\marginpar{Лемма \ref{UpBottau_spaces} налагает дополнительные ограничения $\tau \leq \lfloor\frac{L-r}{2}\rfloor$ и $\tau \leq K-r$ на $\tau$ и ранг $r$ в алгоритме}
...
Пусть $\tX^{(1)}$ и $\tX^{(2)}$ --- бесконечные ряды, управляемые ЛРФ порядка $d$. Если отрезки этих бесконечных рядов достаточно большой длины $N$ такой, что сдвиг $0 < \tau \leq \min \bracket{\lfloor\frac{L-d}{2}\rfloor, K-d, \lfloor\frac{K}{2}\rfloor}$, \marginpar{check}
