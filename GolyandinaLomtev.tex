...
\textbf{Вход}: Матрица $\matr{Y}$ размера $L \times K$ ранга $r$, сдвиг $0 < \tau \leq \lfloor\frac{K}{2}\rfloor$.
\begin{enumerate}
...
\marginpar{Стоит ли явно указать, что ЛРФ минимальная}
...
\begin{lemma}
	...
	Если ряд $\tX_N$ управляется ЛРФ порядка $d$, $\tau \leq \min\bracket{K - d, \lfloor \frac{L - d}{2} \rfloor}$, то
	$
...
Пусть $\tX_N=\tX_N^{(1)}+\tX_N^{(2)}$, $\matr{Y}^{(1)}$ и $\matr{Y}^{(2)}$ --- траекторные матрицы рядов $\tX_N^{(1)}$ и $\tX_N^{(2)}$, $\tau \leq \min\bracket{\lfloor \frac{K}{2} \rfloor ,K - d, \lfloor \frac{L - d}{2} \rfloor}$.

\marginpar{добавил условие на $\tau$}
