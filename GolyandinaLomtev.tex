...
\begin{document}
    \begin{center}
      ...
      {Голяндина Н.Э.\footnote{Работа поддержана грантом РФФИ номер 15-04-06480}, Ломтев М.А.}\\[0.4cm]
...
сильная разделимость отсутствует.\footnote{reference} Возникает задача построения другого оптимизационного критерия, который
...
\begin{definition}
\label{def:sep_w}
	Ряды $\tX^{(1)}$ и $\tX^{(2)}$ назовём слабо $L$-разделимыми методом A, если существует такое разложение траекторной матрицы $\matr{X}$ на элементарные, полученное методом A, что его можно разбить на сумму матриц $\matr{X}^{(1)}$ и $\matr{X}^{(2)}$.
\end{definition}

\begin{definition}
\label{def:sep_s}
	Ряды $\tX^{(1)}$ и $\tX^{(2)}$ назовём сильно $L$-разделимыми методом A, если любое разложение траекторной матрицы $\matr{X}$ на элементарные, полученное методом A, можно разбить на сумму матриц $\matr{X}^{(1)}$ и $\matr{X}^{(2)}$.
\end{definition}
...
Заметим, что в \cite[разделы 1.5 и 6.1]{Golyandina.etal2001}, где рассматривается Basic SSA,
утверждения \ref{prop:sep_w} и \ref{prop:sep_s} взяты в качестве определений разделимости, а
определения \ref{def:sep_w} и \ref{def:sep_s} являются эквивалентными свойствами.
...
\begin{definition}
	Ряд $\tX_N$ называется рядом конечного ранга $d \leq \min\bracket{L,K}$, если его $L$-траекторная матрица имеет ранг $d$ при любом $L \geq d$.
\end{definition}
\begin{definition}
...
Таким образом, на первом этапе мы делаем обычное сингулярное разложение $L$-траекторной матрицы $\matr{X}$. Выделяем группу неразделившихся компонент с индексами $I$, $|I|=r$. Обозначим $\matr{Y}=\left[Y_1:\ldots:Y_K\right]=\matr{X}_I$ сгруппированную матрицу этих компонент размерности $L \times K$ ранга $r$. Метод SSA-AMUSE строит разложение матрицы $\matr{Y}=\matr{Y}^{(1)}+\ldots+\matr{Y}^{(r)}$. Затем так же, как в Basic SSA, получаем ряды диагональным усреднением:
$
\wtilde\tY_N = \wtilde\tY^{(1)}_N+\ldots+\wtilde\tY^{(r)}_N,\ \  \wtilde\tY^{(k)}_N = \calT^{-1} \calH \matr{Y}_{k}.
$
Для выделения компонент применяется группировка.
...
Пусть $\matr{B}$ --- произвольная матрица из $\cM_{A,B}$ и $\tau$ --- некоторое положительное целое число. Обозначим $\toptau{\matr{B}}$ матрицу $\matr{B}$ без первых $\tau$ строк, $\bottau{\matr{B}}$ матрицу $\matr{B}$ без последних $\tau$ строк, $\lefttau{\matr{B}}$ матрицу $\matr{B}$ без первых $\tau$ столбцов, $\righttau{\matr{B}}$ матрицу $\matr{B}$ без последних $\tau$ столбцов при $0 < \tau < A,\: 0 < \tau < B$.

Положим
	\begin{gather}
	\label{C_tau}
\matr{C}_{\tau}\bracket{\matr{B}} = \frac{1}{2}\left[\T{\bracket{\toptau{\matr{B}}}}\bottau{\matr{B}}+\T{\bracket{\bottau{\matr{B}}}}\toptau{\matr{B}}\right].
	\end{gather}
...
\begin{lemma}
\label{svd_YY_}
	Пусть $\matr{Y} \in \cM_{L,K}$, $\tau < K-r$. Тогда для любого сингулярного разложения матрицы $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]=\matr{U}\matr{\Lambda}^{1/2}\T{\matr{T}}$,
где $\matr{\Lambda}\in \cM_{r,r}$ --- диагональная матрица с сингулярными числами на диагонали,
$\matr{U}\in \cM_{L,r}$ --- матрица левых сингулярных векторов, и $\matr{T}\in \cM_{2K-2\tau,r}$ --- матрица правых сингулярных векторов,
верно, что $\matr{T}=\left[\toptau{\matr{Q}}:\bottau{\matr{Q}}\right]$, где $\matr{Q}=\T{\matr{Y}}\matr{U}{\matr{\Lambda}}^{-1/2}$.
\end{lemma}

\begin{proof}
	Сингулярное разложение матрицы $\left[\!\lefttau{\matr{Y}}\!:\!\righttau{\matr{Y}}\!\right]$ можно записать в виде
	%\begin{gather*}
...
\begin{lemma}
	Пусть матрица $\matr{Q} \in \cM_{K,r}$, и $\matr{C}_{\tau}\bracket{\matr{Q}}=\matr{W}\matr{D}\T{\matr{W}}$ --- спектральное разложение. Тогда для $\matr{S}=\matr{Q}\matr{W}$ матрица $\matr{C}_{\tau}\bracket{\matr{S}}=\matr{D}$ является диагональной.
\label{C_S_diag}
\end{lemma}
\begin{proof}
	Выводится подстановкой $\matr{S}=\matr{Q}\matr{W}$ в $\matr{C}_{\tau}\bracket{\matr{S}}$ и перемножением матриц.
\end{proof}
...
Пусть $\matr{Y} \in \cM_{L,K}$ --- матрица ранга $r$. Тогда $\matr{Y}=\sum\limits_{i=1}^{r}\hat{U}_i \T{S}_i$, где $\hat{U}_i$ --- столбцы матрицы $\hat{\matr{U}}=\matr{U}\matr{\Lambda}^{1/2}\matr{W}$, $\matr{W}$ из спектрального разложения $\matr{C}_{\tau}\bracket{\matr{Q}}=\matr{W}\matr{D}\T{\matr{W}}$, $\matr{Q}=\T{\matr{Y}}\matr{U}\matr{\Lambda}^{-1/2}$, $\matr{U}$ --- левые сингулярные векторы $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]$.
\end{theorem}
\begin{proof}
...
получена на шаге Разложение метода Basic SSA. Алгоритм устроен следующим образом.

\medskip
\textbf{Алгоритм SSA-AMUSE}

\textbf{Вход}: Матрица $\matr{Y}$ ранга $r$, сдвиг $\tau>0$.
\begin{enumerate}
    	\item Сингулярное разложение матрицы $\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]$, которое по лемме \ref{svd_YY_} имеет вид
    		$
    			\left[\lefttau{\matr{Y}}:\righttau{\matr{Y}}\right]=
    				\matr{U}\matr{\Lambda}^{1/2}\left[\T{\bracket{\toptau{\matr{Q}}}}:\T{{\bottau{\matr{Q}}}}\right], \mbox{ где } \matr{\Q}=\T{\matr{Y}}\matr{U}\matr{\Lambda}^{-1/2}.$
    				
    	    \item $\matr{W}=\left[W_1:\ldots:W_r \right]$, где $W_i$, $i=1,\ldots,r$, --- собственные векторы матрицы $\matr{C}_{\tau}\bracket{\matr{\Q}}$.
        \item $\matr{S} = \left[S_1: \ldots :S_r\right]= \matr{\Q}\matr{W}$.
        \item $\hat{\matr{U}}=\left[\hat{U}_1:\ldots:\hat{U}_r\right] = \matr{U}\matr{\Lambda}^{1/2}\matr{W}$.
        \item По теореме \ref{SSA_AMUSE_decomp} $\matr{Y}=\hat{\matr{U}}\T{\matr{S}}$. Поэтому получаем разложение на элементарные матрицы
        \begin{equation}
        	\matr{Y} =\sum\limits_{i=1}^{r}\hat{U}_i\:\T{S}_{i}.
        	\label{decompY}
        \end{equation}
\end{enumerate}
    \textbf{Результат}: разложение матрицы $\matr{Y}$ на сумму элементарных матриц $\matr{Y}=\matr{Y}_1+\ldots+\matr{Y}_r$, где $\matr{Y}_i=\hat{U}_i \T{S}_i$.

\section{Разделимость}

\label{SSA_ICA_separ}

Далее для удобства изложения будем предполагать в этом разделе, что траекторная матрица $\matr{X}$ ряда $\tX_N$
...
матрица $\matr{Y}=\matr{X}_I=\matr{X}$ и имеет ранг $r=d$. В общем случае, группа $I$ может соответствовать компоненте ряда,
сильно отделимой от остатка.

%\subsection{Свойства подпространств}
\subsection{Вспомогательные утверждения}
\label{SSA_ICA_prop}
Будем рассматривать ряды конечного ранга, управляемые ЛРФ.
Докажем несколько утверждений о свойствах траекторных пространств.

Как и раньше,  пусть $\gX^{(K,1)},\gX^{(K,2)}$ и $\gX^{(K)}$ --- пространства строк траекторных матриц рядов $\tX_N^{(1)},\:\tX_N^{(2)}$ и $\tX_N$ размерности, соответственно, $d_1,d_2$ и $d$. Пусть $\righttau{{\gX}^{(K,1)}},\:\righttau{{\gX}^{(K,2)}}$ и $\righttau{{\gX}^{(K)}}$ --- пространства строк матриц без последних $\tau$ столбцов, и также $\lefttau{{\gX}^{(K,1)}}$, $\lefttau{{\gX}^{(K,2)}}$ и $\lefttau{{\gX}^{(K)}}$ --- пространства строк матриц без первых $\tau$ столбцов.

\begin{lemma}
	\label{UpBottau_spaces}
	Если ряд управляется ЛРФ ранга $d$, $d \leq \min\bracket{K,L - 2\tau}$, то
	$
...
Рассмотрим базис $V_1,\ldots,V_d$ в $\gX^{(K)}$.
Соответствующие ему векторы $\bottau{{V_1}},\ldots,\bottau{{V_d}} \in \righttau{{\gX}^{(K)}}$ линейно независимы, так как иначе их можно было бы продолжить по ЛРФ до
...
\label{one_tau_deter_cons}
	Пусть матрица $\matr{S}=\left[S_1:\ldots:S_d\right]$ имеет ранг $d$, причём для фиксированного $\tau > 0$ матрица $\matr{C}_{\tau}\bracket{\matr{S}}$ является диагональной: $\matr{C}_{\tau}\bracket{\matr{S}}=\func{diag}\bracket{\mu_1,\ldots,\mu_d}$, $\mu_1 \geq \mu_2 \geq \ldots \geq \mu_d$,
   матрица $\matr{W}$ ортогональна, $\matr{T}=\matr{S}\T{\matr{W}}$. Если для некоторых $I_1$ и $I_2$ множества $\lbrace \mu_i, i \in I_1 \rbrace$ и $\lbrace \mu_j, j \in I_2 \rbrace$ не пересекаются, то для любого спектрального разложения матрицы $\matr{C}_{\tau}\bracket{\matr{T}}=\matr{H}\matr{D}\T{\matr{H}}$ с собственными числами, упорядоченными по убыванию, выполнено
	\begin{enumerate}
		\item $\matr{D}=\matr{C}_{\tau}\bracket{\matr{S}}$,
		\item $\func{span}\bracket{H_k: k \in I_1}=\func{span}\bracket{W_l: l \in I_1}$ и $\func{span}\bracket{H_k: k \in I_2}=\func{span}\bracket{W_l: l \in I_2}$, где $H_i$, $i=1,\ldots,d$, --- столбцы матрицы $\matr{H}$, $W_j$, $j=1,\ldots,d$, --- столбцы матрицы $\matr{W}$.
	\end{enumerate}
\end{lemma}
\begin{proof}
	По лемме \ref{C_S_diag} матрицы $\matr{C}_{\tau}\bracket{\matr{T}}$ и $\matr{C}_{\tau}\bracket{\matr{S}}$ подобны, поэтому у них совпадают собственные числа, что доказывает первый пункт леммы. Столбцы матрицы $\matr{W}$ являются ортонормированными собственными векторами матрицы $\matr{C}_{\tau}\bracket{\matr{T}}$.	
	Так как подмножества собственных чисел не пересекаются, то подпространства собственных векторов, соответствующие этим подмножествам, определены однозначно. Векторы $W_i$, $i \in I_1$, и $W_j$, $j \in I_2$, являются ортогональными базисами этих подпространств, а векторы $H_i$, $i \in I_1$, и $H_j$, $j \in I_2$, образуют другие ортогональные базисы этих же подпространств.
\end{proof}

\subsection{Условия разделимости}
\label{SSA_ICA_sep_cond}

Пусть $\tX_N=\tX_N^{(1)}+\tX_N^{(2)}$, $\matr{Y}^{(1)}$ и $\matr{Y}^{(2)}$ --- траекторные матрицы рядов $\tX_N^{(1)}$ и $\tX_N^{(2)}$.
...
\begin{proof}
	...
	Необходимость. Из разделимости следует, что существует базис $\left\lbrace S_1,\ldots, S_d \right\rbrace$ пространства $\gX^{(K)}$, разбивающийся на подмножества $\left\lbrace S_1^{(1)},\ldots, S_{d_1}^{(1)} \right\rbrace$ и $\left\lbrace S_1^{(2)},\ldots, S_{d_2}^{(2)} \right\rbrace$ так, что первое составляет базис $\gX^{(K,1)}$, а второе --- базис $\gX^{(K,2)}$. Причём, если обозначить $\matr{S}^{(i)}=\left[ S_1^{(i)}:\ldots: S_{d_i}^{(i)} \right],\: i=1,2$, $\matr{S}=\left[ \matr{S}^{(1)}: \matr{S}^{(2)} \right]$ (переупорядочили векторы для удобства), то матрица $\matr{C}_{\tau}\bracket{\matr{S}}$ диагональна. Обозначим $\Xi^{(i)} = \T{\left[\!\T{\bracket{\toptau{{\matr{S}}^{(i)}}}}\!:\!\T{\bracket{\bottau{{\matr{S}}^{(i)}}}}\!\right]}$. Столбцы матрицы $\Xi^{(1)}$ ортогональны столбцам $\Xi^{(2)}$, при этом столбцы матрицы $\Xi^{(i)}$ образуют ортогональный базис строк матрицы $\left[\!\lefttau{{\matr{Y}}^{(i)}}:\righttau{{\matr{Y}}^{(i)}}\!\right]$. Отсюда следует равенство \eqref{eq:wsep1}.
	
	Используя условие разделимости, можно показать, что матрица $\matr{P}^{(i)}\!=\!\matr{U}\!\matr{\Lambda}^{1/2}\matr{W}^{(i)}\!\T{\bracket{\!\matr{W}^{(i)}\!}}\!\!\matr{\Lambda}^{-\frac{1}{2}}\T{\matr{U}}$ задает ортогональный проектор на пространство столбцов матрицы $\matr{Y}^{(i)}$.
%и поэтому $\matr{Y}^{(i)}=\matr{P}^{(i)}\matr{Y}=\matr{P}^{(i)}\matr{P}^{(i)}\matr{Y}=\matr{P}^{(i)}\matr{Y}^{(i)}$.
	Воспользовавшись этим, выведем второе условие из диагональности матрицы $\matr{C}_{\tau}\bracket{\matr{S}}$. Матрица может быть записана в виде
	\begin{eqnarray*}
		\matr{C}_{\tau}\bracket{\matr{S}} = \left(\begin{array}{cc}
		\matr{C}_{\tau}\bracket{\matr{S}^{(1)}} & \matr{C}^{(1,2)} \\
		\matr{C}^{(1,2)} & \matr{C}_{\tau}\bracket{\matr{S}^{(2)}}
	\end{array}\right),
	\end{eqnarray*}
	где
...
\lefttau{{\matr{Y}}^{(1)}}\T{\bracket{\righttau{{\matr{Y}}^{(2)}}}}\right]\matr{U}\!\matr{\Lambda}^{-1/2}\matr{W}^{(2)}/2$ и равно нулю по условию диагональности.
	Домножим справа на $\T{\bracket{\matr{W}^{(i)}}}\matr{\Lambda}^{-1/2}\T{\matr{U}}$ и слева на $\matr{U}\matr{\Lambda}^{1/2}\matr{W}^{(i)}$, воспользуемся свойствами проекторов $\matr{P}^{(i)}$ и получим условие \eqref{eq:wsep2}.
	
	Достаточность. По условию \eqref{eq:wsep1} и по лемме \ref{svd_YY_} существуют базисы $\matr{S}^{(1)} \in \gX^{(K,1)}, \matr{S}^{(2)} \in \gX^{(K,2)}$ такие, что столбцы матрицы $\Xi^{(1)}$ ортогональны друг другу и столбцам $\Xi^{(2)}$.
Отсюда можно вывести, что столбцы матрицы $\matr{S}=\left[ \matr{S}^{(1)}: \matr{S}^{(2)}\right]$ образуют базис $\gX^{(K)}$.	
	Из условия \eqref{eq:wsep2} следует, что матрица $\matr{C}_{\tau}\bracket{\matr{S}}$
	 диагональна. Далее остаётся спроектировать траекторные матрицы на найденный базис и найти элементарные матрицы:
$
...
Пусть также $\matr{Q}^{(1)}$ --- некоторый базис для первого ряда, $\matr{Q}^{(2)}$ --- некоторый базис для второго ряда, такие что
			\begin{gather*}		
				\left[\lefttau{\matr{Y}^{(i)}}:\righttau{\matr{Y}^{(i)}}\right]=
    				\matr{U}^{(i)}\bracket{\matr{\Lambda}^{(i)}}^{1/2}\left[\T{\bracket{\toptau{\bracket{\matr{\Q}^{(i)}}}}}:\T{\bracket{\bottau{\bracket{\matr{\Q}^{(i)}}}}}\right], i=1,2,
    		\end{gather*}
    		--- сингулярные разложения.
Тогда сильная SSA-AMUSE разделимость равносильна тому, что
 собственные числа матриц $\matr{C}_{\tau}\bracket{\matr{Q}^{(1)}}$ и $\matr{C}_{\tau}\bracket{\matr{Q}^{(2)}}$
не пересекаются.
%	\end{itemize}
\end{theorem}
\begin{proof}
...
Пусть $\tX^{(1)}$ и $\tX^{(2)}$ --- бесконечные ряды, управляемые ЛРФ. Если отрезки этих бесконечных рядов длины $N$
слабо разделимы в Basic SSA при длине окна $L$, то удлинённые на $\tau$ (то есть длины $N+\tau$) отрезки ряда будут слабо SSA-AMUSE разделимы при том же $L$ и сдвиге $\tau$. Также будут слабо SSA-AMUSE разделимы неудлинённые отрезки длины $N$ при длине окна $L-\tau$ и сдвиге $\tau$.
\end{proposition}
...
\begin{enumerate}
	...
	\label{sinsin_str_ex}
	Рассмотрим $\tX^{(1)}=(\cos \pi n, n=1,\ldots,N)$ и $\tX^{(2)}=(\func{const}, n=1,\ldots,N)$. Если $K$ нечётно, то пространства не ортогональны, однако условия \eqref{eq:wsep1} и \eqref{eq:wsep2} выполнены. Таким образом, данные ряды слабо SSA-AMUSE разделимы.
	
	\item Получим условия сильной разделимости двух гармоник.
	Рассмотрим ряды $\tX^{(1)}$ и $\tX^{(2)}$ длины $N$ с общими членами в виде $x^{(i)}_n=A_i\sin \bracket{2\pi n \omega_i + \gamma_i}$, $n=1,\ldots,N$, $0<\omega_i \leq 0.5$, $\omega_1\neq\omega_2$. Докажем, что если $(K-1)\omega_1$ и $(K-1)\omega_2$ целые, то ряды сильно SSA-AMUSE разделимы. Слабая разделимость следует из слабой разделимости в Basic SSA (см. утверждение~\ref{prop:sepSSAandAMUSE}). Сильную разделимость можно доказать  следующим образом. Пусть для определенности $\omega_i<0.5$. В этом случае найденные на шаге 3 алгоритма SSA-AMUSE базисы будут ортогональны и иметь вид:
	\begin{gather*}
	...
	К тому же, для таких базисов каждая из матриц $\matr{C}_{\tau}\bracket{\matr{S}^{(i)}}$, $i=1,2$, диагональна, при этом на диагоналях стоят числа ${\cos\bracket{2\pi \omega_i}}/{2}$, они же являются собственными числами этих матриц. Поэтому условия сильной разделимости в теореме~\ref{strong_separ}
 выполнены.

Заметим, что в случае SSA-AMUSE компоненты будут упорядочены по значениям $\cos\bracket{2\pi \omega_i}$, в то время как в Basic SSA они упорядочены по амплитудам $A_i$. Упорядоченность по $A_i$ приводит к смешиванию в случае совпадающих амплитуд.
...
Конечно, в реальных задачах условия точной разделимости редко выполнены и имеет место приближенная разделимость,
которая является следствием наличия асимптотической разделимости.

Понятие асимптотической SSA-AMUSE слабой разделимости определяется аналогично асимптотической разделимости
...
Условием сильной разделимости в SSA-AMUSE является дизъюнктность множеств собственных чисел матриц $\matr{C}_{\tau}\bracket{\matr{S}^{(K,i)}}$, где $\matr{S}^{(K,i)}$ --- матрицы, чьи столбцы содержат базисы, найденные на шаге 3 алгоритма SSA-AMUSE.
...
чисел матриц $\matr{C}_{\tau}\bracket{\matr{S}^{(K,i)}}$ при $K\rightarrow\infty$, если такие пределы существуют.

Метод SSA-AMUSE имеет преимущество и в асимптотической сильной разделимости.
А именно, гармоники $\tX^{(1)}$ и $\tX^{(2)}$  с общими членами в виде $x^{(i)}_n\!=\!A_i\sin\! \bracket{2\pi n \omega_i + \gamma_i}$, $n=1,\ldots,N$, являются асимптотически сильно разделимыми даже при совпадающих
амплитудах при $K\rightarrow \infty$. Это доказывается непосредственным вычислением собственных чисел матриц $\matr{C}_{\tau}\bracket{\matr{S}^{(K,i)}}$.%


\section{Сравнение}
\label{Comparison}

Так как оба метода, предлагаемый в статье SSA-AMUSE и введенный в \cite{DerivSSA} DerivSSA, используются для ослабления условий сильной разделимости, возникает задача их сравнения.
...
%	\captionsetup{labelformat=empty}
	\caption{RMSE для оценки $\sin(2\pi/7)$ в сумме  $\sin(2\pi/7)+\sin(2\pi \omega_2)$ в зависимости от  $\omega_2$.}
\label{fig:comp}
\efg
...
Продемонстрируем это на численном эксперименте. На рисунке \ref{fig:comp} изображены ошибки RMSE оценки одной гармонической компоненты по наблюдаемой сумме двух синусов. Параметр $\omega_2$ меняется от 0.01 до 0.25 с шагом 0.001. Естественно, что при $\omega_2\approx \omega_1=1/7$ разделимости нет. Однако видно, что область с большой ошибкой вокруг $\omega_2=1/7\approx 0.14$  более узкая для SSA-AMUSE.
